...
def log_to_sheets(user_input, ai_response, calories, protein, water):
    ...
    try:
        ...
        # לא מציגים שגיאה למשתמש כדי לא להפריע לחוויה, רק אם את רוצה לעקוב
        pass
...
with col1:
    ...
    st.progress(min(st.session_state.current_calories / calorie_target, 1.0) if calorie_target > 0 else 0.0)
    st.write(f"{st.session_state.current_calories} / {calorie_target}")
with col2:
    st.subheader("🥩 חלבון")
    st.progress(min(st.session_state.current_protein / protein_target, 1.0) if protein_target > 0 else 0.0)
    st.write(f"{st.session_state.current_protein} / {protein_target}")
with col3:
    st.subheader("💧 מים")
    st.progress(min(st.session_state.current_water / water_target, 1.0) if water_target > 0 else 0.0)
...
if user_input:
    ...
    system_instruction = f"אתה מאמן תזונה. עליך לנתח: {user_input}. החזר JSON עם: response (תגובה), added_calories (מספר), added_protein (מספר), added_water (מספר)."
